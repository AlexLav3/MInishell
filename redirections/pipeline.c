...
/**
 * Counts the number of commands separated by pipe tokens ("|") in a linked list of tokens.
 * Returns 0 if the token list is empty.
 * Starts counting from 1 since at least one command exists if tokens are present.
 * Iterates through the tokens, incrementing count each time a pipe token is found.
 */
 ...
 * Temporarily disconnects the previous token from the current to isolate the command segment.
 * Parses the arguments and redirections for the isolated command segment and stores them in ctx->cmd->args.
...
/**
 * Builds an array of commands (cmds) from a linked list of tokens split by pipes.
 * Iterates through tokens and processes each segment separated by pipe tokens.
 * For each pipe token encountered, extracts and parses the corresponding command arguments and redirections.
 ...
 */
static	void	build_cmds_from_tokens(t_token *tokens, t_cmd *cmds,
								int cmd_count,t_shell *shell)
{
	...
	if (cmd_i < cmd_count && *(ctx.start))
		cmds[cmd_i].args = parse_args_and_redirs(*(ctx.start), &cmds[cmd_i], shell);
}
 ...
 * Cleans up allocated resources: frees command arguments arrays and resets redirections, then frees the commands array.
  */
